...
1. **0b (second half)** — confirm RB2B's webhook/CSV export carries the same `_reb2buid` UUID so the join closes on the Reach side (client side confirmed; see Closed table). Downgraded from blocker: capture proceeds regardless; this only decides whether the join is ID→ID or falls back to RB2B→email→Reach.
2. **0f** — does Webflow Cloud honor `triggers.crons`? Test empirically on the first staging deploy; expected no → ship the GitHub Actions nightly cron. Trigger mechanism only; `/e/push` unchanged either way.
