...
- **Fit-assessment app** — reviewed Aug 1 2026 (already Next-wrapped, basePath `/fit-assessment`, own tracking + own `/api/leads`→Reach). **No conflicts with the identity server, but zero integration**: no visitor/session IDs, its own localStorage first-touch (re-inheriting the Safari ITP problem), no retry/dedupe/bot-flagging on its lead path, pageviews invisible to the touch log (fuzzy-join holes once RB2B sees its pages), and a second independent ad-pixel conversion path (IDs still REPLACE_ME — decide ONE conversion home before configuring them; GTM `qualified_lead` is the incumbent). **Consolidation (small, do at its launch if identity is live first, else fast-follow):** POST `/e/v` on load + step transitions (virtual paths `/fit-assessment/step/N` — it's a state-machine SPA, instrument `setStep`, no history-API hooks needed); replace `/api/leads` with `/e/collect` (`form_id: "fit-assessment"`, assessment answers as passthrough) and delete its Reach route + `REACH_API_KEY`; drop `pf_fit_first_touch`. Until consolidated, Reach treats `source: fit_assessment` as a third path with no dedupe handle. Repo transfers to the Popfly org at launch.
